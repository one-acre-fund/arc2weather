...
extractWeatherData <- function(rawData, 
  ...
  if(is.null(yearRange) && is.null(monthRange)){
    stop("\n Please provide either a year or a month input for query")
  }
  
  datExtract <- extract_velox_gps(
    veloxRaster = convert_to_velox(subsetYearMonth(rawData, yearRange, monthRange)), 
...
```{r}
datExport <- extract_velox_gps(veloxRaster = convert_to_velox(subsetYear(arcWeatherRaw, 1989)), spdf = convertSpdf(combineGpsFiles, "Longitude", "Latitude"))
...
```{r}
output <- extractWeatherData(arcWeatherRaw, yearRange = 1989, combineGpsFiles, latCol = "Latitude", lonCol = "Longitude")
```
