...
1/3/19 - Now that the functions are working I want to test that the set up will work with data coming in from the warehouse. This probably means:

* having to identify or assign CRS to data from the warehouse wihtout it
* work with different variable names of labels and ids. I want to be indifferent to the incoming labels. (I think we are).
* Test how to set up this system with Mandela!
...
subsetYearMonth <- function(rList, year = NULL, month = NULL){
...
```{r, eval=FALSE}
test <- subsetYearMonth(arcWeatherRaw, 1987)
#test <- subsetYear(arcWeatherRaw, 1987)
...
oafTrim <- function(veloxObj){
  # input: list of velox raster but use the first element of the object. The input will be a list of two elements with the first elemetn being the list we want to iterate over.
  ...
  # notes: This step might be unncessary? In that case we can probably pretty easily remove it from the sequence.
    ...
    for(i in seq_along(veloxObj[[1]])){
      veloxObj[[1]][[i]]$crop(oafAreaReproject)
    }
    return(veloxObj)
}
```
...
```{r}
convertSpdf <- function(df, lat = NULL, long = NULL, defaultCRS = NULL){
...
alignCRS <- function(veloxRasterList, spdf){
...
veloxRaster <- convert_to_velox(subsetYear(arcWeatherRaw, 1989))
spdf <- convertSpdf(combineGpsFiles, "Longitude", "Latitude")


test <- extract_velox_gps(veloxRaster, spdf)
table(test$rainfall)
```


## Functions to tidy up data

```{r}
tidyArcWeatherData <- function(df){
...
## Function for full extract

**Come back and add in year and month filters**

```{r}
extractWeatherData <- function(rawData, 
                               gpsFile, 
                               latCol, 
                               lonCol){
...
#test <- convertSpdf(combineGpsFiles, "Longitude", "Latitude")
siteSp <- convertSpdf(combineGpsFiles, "Longitude", "Latitude")
...
* convert to spdf.
* align CRS.
* extract the data for the points in the spdf.
* prep data for inport into warehouse so that it can be summarized as desired.

Start with `combineGpsFiles` and `arcWeatherRaw` and get to dataset with extracted values and labels. This is what we'd send back to a table in the data warehouse for aggregation as we need to combine with other 

**For some reason I'm only getting Nas when I run this as a set of functions. Come back and check this**

```{r}
datExport <- extract_velox_gps(veloxRaster = convert_to_velox(arcWeatherRaw), spdf = convertSpdf(combineGpsFiles, "Longitude", "Latitude"))


#table(datExport$rainfall)
```

I'm giving up on subsetting the data in the function and propose that we instead extract for all available dates and filter in SQL. The downside is that it'll take longer to run. We can always feed in the subset functions but not make them required.

```{r}
output <- extractWeatherData(subsetYear(arcWeatherRaw, 1989), combineGpsFiles, latCol = "Latitude", lonCol = "Longitude")
```


# Velox notes - appendix
