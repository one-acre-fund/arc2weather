...
checkCRS <- function(veloxRasterList, spdf){
  ...
  if(!crsFormatCheck){
    spdf <- spTransform(spdf, CRS(veloxRaster[[1]]$crs))
  }
}
...
extract_velox_gps <- function(veloxRaster, spdf){
  ...
  # ***** this is where I need to come back and figure out how to combine the labels with the extracted data so I can aggregate easily!
  
  extractDf <- do.call(cbind, extractLoop)
  
  fullDf <- cbind(spdf@data, extractDf)
...
fullerTest2 <- extract_velox_gps(fullerTest[[1]],siteSp)
```
