...
checkCRS <- function(veloxRasterList, spdf){
  ...
  if(!veloxCrsCheck){
    stop("\n VeloxRaster object has different CRS. Check this before proceeding")
  }
  
  
  crsFormatCheck <- veloxRasterList[[1]]$crs == proj4string(spdf)
  if(!crsFormatCheck){
    spdf <- spTransform(spdf, CRS(veloxRaster$crs))
    return(spdf)
  }
}
...
extract_velox_gps <- function(veloxRaster, spdf, varsToKeep = NULL){
  # input: v$extract_points(siteReproject) style code. 
  # Takes a list of velox raster and extracts the right data points
  # output: a list with two elements: the first element is a list of velox rater values for the given gps points. The second list is the df of GPS points to later merge with those GPS points for the final output.
  
  # This checks which variables we want to keep from the spdf and keep those for future manipulation
  ...
  spdf <- checkCRS(veloxRaster, spdf)
  
  ## here is where we extract the values for the given GPS values.
  extractLoop <- list()
  for(i in seq_along(veloxLoop)){
    extractLoop[[i]] <- veloxLoop[[1]]$extract_points(spdf)
  }
  
  # return both the extracted values and the spdf of data so I can easily combine them again later.
  return(list(extractLoop, spdf))
...
This will use the second list element from the `convert_to_velox` function to identify how to aggregate the velox list.

```{r}
fullerTest <- convert_to_velox(subsetYearMonth(arcWeatherRaw, 1987, 11))
