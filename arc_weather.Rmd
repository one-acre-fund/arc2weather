...
tidyArcWeatherData <- function(df){
  ...
  clean <- df %>%
    mutate(rainfall = ifelse(rainfall == -999, NA, rainfall))
...
```{r}
datExport <- tidyArcWeatherData(extract_velox_gps(oafTrim(convert_to_velox(subsetYear(arcWeatherRaw, 1985))), convertSpdf(combineGpsFiles, "Longitude", "Latitude")))
